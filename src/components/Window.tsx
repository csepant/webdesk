...
export default function WindowComponent({ title, subtitle, content, open, handler, parentRef }: { title: string; subtitle: string; content: React.ReactNode, open?: boolean, handler?: () => void, parentRef?: React.RefObject<HTMLDivElement | null> }) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div className="relative h-full bg-slate-900/95 backdrop-blur-sm p-6 overflow-hidden"
          style={{
            clipPath: 'polygon(0 0, calc(100% - 30px) 0, 100% 30px, 100% 100%, 0 100%, 30px 100%, 0 calc(100% - 30px))',
          }}
        >
          <div className="px-4 text-sm font-mono text-sky-500  mb-2">
            <div data-testid="window-controls" className="absolute top-2 right-10 flex space-x-2">
              <div className="float-right flex items-center h-8 w-8 text-green-200 space-x-2 bg-green-500/50 p-2 rounded-full cursor-pointer" onClick={() => {
                handleMaximize();
              }}
                onTouchStart={() => {
                  handleMaximize();
            ...
            <div data-testid="window-title" className="font-bold text-white mt-2">
              {title}
            </div>

          </div>

          <div className=" text-sky-500 px-4 py-2 font-semibold">
            {subtitle}
          </div>

          <div className="p-4 bg-transparent white/10 border-t border-sky-600/50 text-white overflow-y-auto h-[calc(100%-120px)] border-b">
            {content}
          </div>
